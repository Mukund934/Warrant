...
export function deploymentScopedNonceStore(
  ...
  describe(`the contract a deployment-scoped store must satisfy — ${label}`, () => {
    it(
      "refuses a nonce spent against any other instance",
      async () => {
    ...
    it(
      "claims each distinct nonce exactly once across instances",
      async () => {
        const { first, second } = await pair();
    ...
    it(
      "claims a contested nonce exactly once under concurrency",
      async () => {
        const { first, second } = await pair();
